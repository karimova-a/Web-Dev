...
export class ProductListComponent {
  ...
  favouritesCount = computed(() => this.favouriteIds().length);

  favouriteProducts = computed(() => this.favouriteIds().length);
  ...
  showFavouritesOnly = signal(false);

  displayedProducts = computed(() => {
    if (this.showFavouritesOnly()) {
      return this.products.filter(p => this.favouriteIds().includes(p.id));
    }
